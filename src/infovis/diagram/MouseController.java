...
import infovis.diagram.layout.Layout;//neu
...
public class MouseController implements MouseListener,MouseMotionListener {
	...
	public void mouseDragged(MouseEvent e) {
		...
		int x_scale = (int)(x * overviewScale);
		int y_scale = (int)(y * overviewScale);
		...
		if(view.markerContains(x_scale, y_scale)) {
			view.updateTranslation((x-mouseOffsetX)/scale, (y-mouseOffsetY)/scale);
			view.updateMarker((int)(((x - mouseOffsetX)*overviewScale)/scale), (int)(((y - mouseOffsetY)*overviewScale)/scale));
		}
		else if(view.overviewContains(x_scale, y_scale)) {
			System.out.print("HIT OVERVIEW\n");
			view.updateOverview(((x - mouseOffsetX)*overviewScale)/scale, ((y - mouseOffsetY)*overviewScale)/scale);
		}
		if (fisheyeMode){ // Wenn fisheymode ... 
			Model reset = new Model();
			reset.generateTestValues();// Hole test values 
			view.setModel(reset);// Setzen als view model 
			fisheye.setMouseCoords(e.getX(), e.getY(), view);//Setze die Mauscoordinaten mit bzw im fisheyelayout 
			view.repaint(); // Zeichne es neu
		} 
	...
	public void setFisheyeMode(boolean b) { // Setze fisheyemode
		fisheyeMode = b;
		Model reset = new Model();//Model initialisierung
		reset.generateTestValues();//mit testvalues
		
		if (b){ // Wenn fisheymode ...
			Debug.p("new Fisheye Layout");
			view.setModel(fisheye.transform(reset, view));// Gesetztes Modell wird transformiert 
			view.repaint(); // und neu gezeichnet
		} 
		else {
			Debug.p("new Normal Layout");
			//view.setModel(model);
			view.setModel(reset);// Modell gesetzt
			view.repaint();
		}
	}
